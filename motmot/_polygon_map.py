...
def make_polygon_map(ids: np.ndarray, id_count: int, use_C=True):
    ...
    _polygons, _sub_ids = np.divmod(np.arange(ids.size, dtype=np.intp),
                                    ids.shape[1])
    _dest_vertices = ids[_polygons, (1 + _sub_ids) % d]

    polygons, dest_vertices = \
        RaggedArray.groups_by(ids.ravel(), _polygons, _dest_vertices,
                              id_max=id_count)
    ...
    polygon_map = np.empty(ids.shape, dtype=np.intp, order="C")
    if use_C:
        populate_polygon_map_c(polygon_map, ids, n, d, dest_vertices, polygons)
    else:
        populate_polygon_map_py(polygon_map, ids, n, d, dest_vertices, polygons)

    return polygon_map


def populate_polygon_map_c(polygon_map, ids, n, d, dest_vertices: RaggedArray,
                           polygons: RaggedArray):
    assert ids.dtype == np.intp
    assert ids.flags.c_contiguous
    slug.dll.populate_polygon_map(ptr(polygon_map), ptr(ids), n, d,
                                  dest_vertices._c_struct._ptr,
                                  polygons._c_struct._ptr)


def populate_polygon_map_py(polygon_map, ids, n, d, dest_vertices, polygons):
...
if __name__ == "__main__":
    ids = np.array([[0, 1, 2, 5], [3, 4, 0, 5], [2, 1, 0, 6]], dtype=np.intp)
    id_count = ids.max() + 1
    target = np.array([[2, 2, -1, 1], [-1, -1, 0, -1], [0, 0, -1, -1]])
    assert np.array_equal(make_polygon_map(ids, id_count, use_C=True), target)
