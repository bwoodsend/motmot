# Welcome to motmot's test suite!
...
motmot aims to be out-the-box compatible with
...
that PyInstaller does need special help when compiling motmot
then that *help* goes in a *hook* file (see
`motmot/__pyinstaller/hook-motmot.py`).
...
changed the motmot source code.
...
This executable is just `pytest.main()` with motmot
...
Otherwise you may unintentionally use the original source motmot
...
If you've modified either the source code or the hooks for motmot
